...
def make_tree(starting_position):
    ...
    tree_trunck = box(pos=vec(0, 0, 0),
       size = vec(0.3, 1, 0.3),color = vec(0.66, 0.33, 0))
       
    tree_leaf = sphere(size = 1.0*vec(1, 1, 1),
                        pos = vec(0, 0.6, 0),
                        color = color.green
                        )

    tree_objects = [tree_trunck, tree_leaf]
    
    com_tree = compound(tree_objects,
                         pos = starting_position
                         )
    return com_tree
...
#
ball = sphere(pos = vec(5,0,5), size = 1.0*vec(1, 1, 1),  # Ball is an object of class sphere
...
tree = make_tree(starting_position = vec(0,0,0))
...
while True:
    ...
    #    alien1.vel.y = 0.0                 # No vertical component of velocity
