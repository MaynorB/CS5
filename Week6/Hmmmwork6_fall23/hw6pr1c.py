...
def test(S):
    """unique takes a list as its only argument and returns
    True if the list has only unique elements (no elements repeated)
    False otherwise (if there are one or more repeats)
    input L: a list
    [[ docstring provided by Amy Yuan! ]]
    """
